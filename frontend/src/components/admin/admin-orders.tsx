...
export default function AdminOrders() {
  ...
  const filterCountActive = useSelector(ordersSelector.selectCountActiveFilter)
  ...
  return (
    <main className={clsx(styles.admin__products, styles.admin__container)}>
      <div className={styles.admin__headerTitle}>
        ...
        <span className={styles.admin__listCount}>{orders.length} заказов</span>
      </div>
        ...
        {({ rowData, columnsData }) => {
          return (
            <Link
              ...
              state={{ background: location }}
            >
              <TableRow rowData={rowData} columnsData={columnsData} />
            </Link>
          )
        }}
      </Table>
