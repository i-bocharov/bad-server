  ...
  return customersData.map((customerFromServer) => ({
    ...
    lastOrderDate: {
      title: customerFromServer.lastOrderDate
        ? format(new Date(customerFromServer.lastOrderDate), 'dd.MM.yyyy')
        : 'Заказ не совершал',
    ...
    lastOrder: customerFromServer.lastOrder
      ? customerFromServer.lastOrder?.products.map((product) => product.title)
      : [],
    ...
    contacts: customerFromServer.lastOrder
      ? [customerFromServer.lastOrder.phone, customerFromServer.lastOrder.email]
      : ['Нет контактов'],
  }))
}
